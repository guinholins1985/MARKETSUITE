...
interface RemarketingCampaignGeneratorProps {
  onBack: () => void;
}

const RemarketingCampaignGenerator: React.FC<RemarketingCampaignGeneratorProps> = ({ onBack }) => {
  useEffect(() => {
    const redirectUrl = 'https://gerador-de-campanhas-de-remarketing.vercel.app/';
    ...
    <div className="py-12 md:py-20 animate-fade-in flex items-center justify-center min-h-[300px]">
        <div className="text-center">
            ...
            <h2 className="text-xl sm:text-2xl font-bold text-slate-100 flex items-center justify-center gap-3 mt-4">
                <ArrowPathRoundedSquareIcon className="h-8 w-8 text-blue-400" />
                Redirecionando
            </h2>
            <p className="mt-3 max-w-2xl mx-auto text-slate-400 sm:text-lg">
                Abrindo o Gerador de Campanhas de Remarketing em uma nova aba...
            </p>
        </div>
    </div>
  );
...
export default RemarketingCampaignGenerator;