...
const PpcAdGenerator: React.FC<PpcAdGeneratorProps> = ({ onBack }) => {
  useEffect(() => {
    const redirectUrl = 'https://geradordeanuncio-ads.vercel.app/';
    ...
    <div className="py-12 md:py-20 animate-fade-in flex items-center justify-center min-h-[300px]">
        <div className="text-center">
            ...
            <h2 className="text-xl sm:text-2xl font-bold text-slate-100 flex items-center justify-center gap-3 mt-4">
                <PresentationChartLineIcon className="h-8 w-8 text-orange-400" />
                Redirecionando
            </h2>
            <p className="mt-3 max-w-2xl mx-auto text-slate-400 sm:text-lg">
                Abrindo o Gerador de Anúncios (PPC) em uma nova aba...
            </p>
        </div>
    </div>
  );
